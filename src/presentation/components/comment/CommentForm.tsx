...
const CommentForm = ()=>{
    ...
    const onSubmit = async (data: Comment) =>{
        try {
            await createComment(data).unwrap(); //trabajar directamente con la data
            console.log(data)
        } catch (error) {
            console.error(error)
        }
    ...
    return(
        <>
        <h1 className="text-amber-400 font-bold">¡NOS INTERESA TU OPINIÓN SOBRE CADA PAÍS!</h1>
        <div className="flex w-auto h-auto justify-center bg-amber-50 gap-4 text-black items-center rounded shadow-amber-100">
            <HiOutlineClipboardDocumentCheck className="bg-black text-white w-30 h-100 rounded" />
            <form className="flex flex-col w-auto h-auto gap-4 m-1.5" onSubmit={handleSubmit(onSubmit)} method="post">
                ...
                <div className="flex flex-col">
                    <textarea className="bg-amber-50 w-100 text-black border-2 rounded" rows={5} {...register('body')} placeholder="Escriba un comentario..."/>
